...
//Η εντολή έρχεται από τον expander σε τακτά χρονικά διαστήματα (1sec) και περιέχει τις τιμές των αισθητήρων και την κατάσταση των ενεργοποιητών
//Οι τιμές αντιγράφονται στις αντίστοιχες μεταβλητές
void s_raw(int arg_cnt, char **args)
{
 if (arg_cnt > 7)
    {
     strcpy(groundTherm1, (char*) args[1]);
     strcpy(groundTherm2, (char*) args[2]);
     strcpy(groundMoist1, (char*) args[3]);
     strcpy(groundMoist2, (char*) args[4]);
     strcpy(flow, (char*) args[5]);
     SgrdTherm1 += atof(groundTherm1);
     SgrdTherm2 += atof(groundTherm2);
     SgrdMoist1 += atof(groundMoist1);
     SgrdMoist2 += atof(groundMoist2);
     Sflow += atof(flow);
     //Τρέχουσα κατάσταση ηλεκτροβάνας 1
     if (atoi((char*) args[6])) //Αν είναι 1
         setSwitchState(1, true);
     else //Αλλιώς είναι 0
         setSwitchState(1, false);
     //Τρέχουσα κατάσταση ηλεκτροβάνας 2
     if (atoi((char*) args[7]))
