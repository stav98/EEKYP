...
//const char *mqtt_server;          //Όνομα του mqtt server π.χ. xxxxx.com
...
void callback(String topic, byte* payload, unsigned int length1)
             {
              ...
              //Διακόπτης Νο 2
              if (topic == (MQTT_CLIENT + "/" + SetA2_TOPIC + "_sub"))
                 {
                  if (msgg == SetA2_ON)
                     { 
...
//Θέτει τον διακόπτη με αριθμό num σε κατάσταση ON ή OFF
//Για δοκιμή MQTT από τερματικό mosquitto_sub -h iot.xxxxxxx.com -u username -P pass -t EEKYP-2A/switch1_pub
 ...
 switch (num)
   {
    case 1:
       ...
       //Μόνο αν άλλαξε η κατάσταση σε κάποιο διακόπτη
           ...
           if (client.connected())
              {
               ...
               client.publish((MQTT_CLIENT + "/" + SetA1_TOPIC + "_sub").c_str(), t1, true);
           ...
           if (client.connected())
              {
               ...
               client.publish((MQTT_CLIENT + "/" + SetA2_TOPIC + "_sub").c_str(), t1, true);
...
static void pubmqtt(void* arg)
{
 ...
 sprintf(t, "%2.1f;%2.1f;%2.2f;%d;%d;%d", MOgrdTherm1, MOgrdTherm2, MOflow, ((Switch1_status == "IsOn")?1:0), ((Switch2_status == "IsOn")?1:0), wifi_strength);
 //Δημοσίευση τώρα
 if (client.connected())
