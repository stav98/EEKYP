...
//Η εντολή έρχεται από τον expander σε τακτά χρονικά διαστήματα και περιέχει τις τιμές των αισθητήρων
 ...
 if (arg_cnt > 5)
    {
     ...
     strcpy(flow, (char*) args[3]);
     SgrdTherm1 += atoi(groundTherm1);
     SgrdTherm2 += atoi(groundTherm2);
     //SgrdMoist1 += atof(groundMoist1);
     //SgrdMoist2 += atof(groundMoist2);
     Sflow += atof(flow);
     
     //Τρέχουσα κατάσταση διακόπτη 1
     if (atoi((char*) args[4])) //Αν είναι 1
         setSwitchState(1, true);
     else //Αλλιώς είναι 0
         setSwitchState(1, false);
     //Τρέχουσα κατάσταση διακόπτη 2
     if (atoi((char*) args[5]))
